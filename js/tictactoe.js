...
function startGame() {
    ...
    if (activePlayer=="") {activePlayer=rollForTurn();}
...
function saveSettings() {
	...
	if (p1Selected[p1Index].text == p2Selected[p2Index].text) {
		alert("We're sorry, but only one Player can be  "+p1Selected[p1Index].text)
	} else {
...
function determineAvatar() {
	...
	if (active == "Player One") { 
		var paintAvatar = p1Avatar;
	} else if (active == "Player Two") {
...
function avatarPlaced() {
	...
	var showPlayer = document.getElementById('showPlayer');
	if (parseText == "That's three in a row, Player One is the winner!" || parseText == "That's three in a row, Player Two is the winner!"){
		showPlayer.innerHTML = "Game Stopped";
		showPlayer.style.color='red';
	}
	activePlayer = showPlayer.innerHTML;
	if (activePlayer == "Player One") {
		showPlayer.innerHTML = "Player Two";
	} else {
		showPlayer.innerHTML = "Player One";
	}
	check4Tie();
}
...
function check4Tie() {
	...
	if(boardState.length >= 9 && check != "That's three in a row, Player One is the winner!" && check != "That's three in a row, Player Two is the winner!") {
		var txt1 = "Oh, it's a tie! You both lose!";
		tieSound();
		writeMsg(txt1);
